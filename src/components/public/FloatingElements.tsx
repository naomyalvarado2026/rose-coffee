...
export default function FloatingElements() {
  ...
  useEffect(() => {
    ...
    const generated: FloatingItem[] = Array.from({ length: 15 }).map((_, i) => {
      ...
      return {
        ...
        size: Math.random() * 16 + 12, // size in px
        delay: Math.random() * 6, // seconds
        duration: Math.random() * 15 + 15, // seconds (slow)
  ...
  const CoffeeBeanIcon = ({ size }: { size: number }) => (
    <svg 
      ...
      viewBox="0 0 24 24" 
      fill="currentColor" 
      className="text-coffee/25 dark:text-coffee/15"
    >
  ...
  const SourdoughIcon = ({ size }: { size: number }) => (
    <svg 
      ...
      viewBox="0 0 24 24" 
      fill="currentColor" 
      className="text-amber-700/20 dark:text-amber-800/10"
    >
      ...
      {items.map((item) => (
        ...
        >
          ...
          {item.type === 'coffee' && (
            <Coffee 
              size={item.size} 
              className="text-coffee/20 dark:text-coffee/10" 
            />
          )}
          {item.type === 'sparkle' && (
            <Sparkles 
              size={item.size - 4} 
              className="text-gold/30 dark:text-gold/15" 
            />
          )}
        </motion.div>
      ))}
